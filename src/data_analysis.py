...
def oov(dataset):
    ...
    """
    train = Data("../data/%s/train.data" % dataset)
    test = Data("../data/%s/test.data" % dataset)
    ...
    with open("../data_analysis/%s/oov.txt" % dataset, "w") as out:
...
def concept_lexicon_mismatch(dataset):
    """
    ...
    """
    train_lex = set(Data("../data/%s/train.data" % dataset).lexicon_clean_concepts)
    test_lex = set(Data("../data/%s/test.data" % dataset).lexicon_clean_concepts)
    ...
    with open("../data_analysis/%s/concept_lexicon_mismatch.txt" % dataset, "w") as out:
...
def embeddings_oov(df_data, path, dataset):
    """
...
if __name__ == "__main__":
    ...
    print("performing data analysis for dataset %s" % dataset)
    # clean up
    os.system("rm -rf ../data_analysis/%s/*" % dataset)

    # plot frequencies of train and test data
    plot_data("../data/%s/train.data" % dataset, "../data_analysis/%s/train/" % dataset, "green")
    plot_data("../data/%s/test.data" % dataset, "../data_analysis/%s/test/" % dataset, "blue")

    # do oov stats
    oov(dataset)
    concept_lexicon_mismatch(dataset)

    # summaries for train and test data
    summaries("../data/%s/train.data" % dataset, "../data_analysis/%s/train/summary.txt" % dataset)
    summaries("../data/%s/test.data" % dataset, "../data_analysis/%s/test/summary.txt" % dataset)

    # check for words missing embeddings
    df_tmp1 = pd.read_pickle("../data/%s/train.pickle" % dataset)
    df_tmp2 = pd.read_pickle("../data/%s/dev.pickle" % dataset)
    df_train = pd.concat((df_tmp1, df_tmp2))
    df_test = pd.read_pickle("../data/%s/test.pickle" % dataset)
    embeddings_oov(df_train, "../data_analysis/%s/train/embeddings.txt" % dataset, dataset)
    embeddings_oov(df_test, "../data_analysis/%s/test/embeddings.txt" % dataset, dataset)
